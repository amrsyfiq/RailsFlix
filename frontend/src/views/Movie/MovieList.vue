...
const filteredMovies = computed(() => {
  ...
  return movieStore.movies.filter(movie =>
    movie.genres.some(g => selectedGenres.value.includes(g.id))
  );
});
...
const toggleAllGenres = () => {
  if (selectAll.value) {
    ...
    selectedGenres.value = movieStore.genres.map(g => g.id);
  }
};
...
<template>
  <BreadCrumb menu="Movie" hrefMenu="movies"/>
  <!-- Start block -->
    ...
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
              ...
              <ul class="space-y-2 text-sm" aria-labelledby="filterDropdownButton">
                <li class="flex items-center">
                  ...
                  <label for="apple" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">
                    All ({{ movieStore.movies.length }})
                  </label>
                </li>
                <li class="flex items-center" v-for="genre in movieStore.genres">
                  <input :id="'genre-' + genre.id" type="checkbox" v-model="selectedGenres" :value="genre.id"
                    class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                  <label for="fitbit" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">
                    {{ genre.name }} ({{ movieStore.movies.filter(movie => movie.genres.some(g => g.id === genre.id)).length }})
                  </label>
                </li>
              </ul>
            </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            <tr v-for="(movie, index) in filteredMovies" class="border-b border-gray-200 dark:border-gray-700">
              <td scope="row" class="px-4 py-3 text-gray-900 dark:text-white flex gap-3 items-center">
                <img :src="movie.image?.file_url ? `http://127.0.0.1:3000${movie.image.file_url}` : 'http://127.0.0.1:3000/default-placeholder.jpg'" alt="Logo" class="object-cover w-16 h-16" />
              ...
              <td class="px-4 py-3">
                <span v-for="genre in movie.genres" class="rounded bg-red-400 text-white px-2 py-0.5 text-sm mr-1 whitespace-nowrap">{{ genre.name }}</span>
              </td>
              <td class="px-4 py-3"> 
                <span class="rounded bg-emerald-500 text-white px-2 py-0.5 text-sm mr-1">{{ movie.status }}</span>
              </td>
              <td class="px-4 py-3">{{ moment(movie.release_date).format('DD/MM/YYYY') }}</td>
              <td class="px-4 py-3">{{ movie.score }}%</td>
              <td class="px-4 py-3">
                ...
                <div v-show="openDropdown === index"
                  ...
                  <ul class="py-1 text-sm">
                    <li>
                      <button type="button" @click="initModal(movie, 'edit')"
                    ...
                    <li>
                      <button type="button" @click="initModal(movie, 'show')"
                        class="cursor-pointer flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
  ...
  <Modal :show="modal.show" @close="modal.show=false">
      ...
      <template #content>
        <MovieForm :data="modal.data" :type="modal.type" @close-modal="modal.show=false"/>
      </template>
  </Modal>
</template>
