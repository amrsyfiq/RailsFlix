...
<template>
  <BreadCrumb menu="Genre" hrefMenu="genres"/>
  <!-- Start block -->
    ...
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            <tr v-for="(genre, index) in genreStore.genres" class="border-b border-gray-200 dark:border-gray-700">
              <td class="px-4 py-3">{{ genre.name }}</td>
              <td class="px-4 py-3 flex justify-end">
                <button type="button" @click="initModal(genre, 'edit')"
                  class="cursor-pointer flex rounded-lg items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                  <i class="fas fa-edit mr-2"></i>
                  Edit
                </button>
                <button type="button" @click="initModal(genre, 'show')"
                  class="cursor-pointer flex rounded-lg items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                  <i class="fas fa-magnifying-glass mr-2"></i>
                  Preview
                </button>
                <button type="button" @click="deleteGenre(genre.id)"
                  class="cursor-pointer flex rounded-lg items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
  ...
  <Modal :show="modal.show" @close="modal.show=false">
      ...
      <template #content>
        <GenreForm :data="modal.data" :type="modal.type" @close-modal="modal.show=false"/>
      </template>
  </Modal>
</template>
